...
class WEDriver:
    ...
    pcoord_ndim : int, default 1
        Dimension of the progress coordinate space.
    pcoord_len : int, default 2
        Length of the progress coordinate time series generated by each segment.
    pcoord_dtype : str or numpy.dtype, default 'float32'
        Data type of the progress coordinate. Must be a valid argument to
        ``numpy.dtype()``.
    ...
    bin_target_counts : int or sequence of int, default 1
        Single target count to use for all the bins, or a sequence of
        target counts, one for each bin.
    ...
    def __init__(
        ...
        *,
        ...
        if rc is not None or system is not None:
            self.system = system or rc.get_system_driver()
            if rc is not None:
                self.rc = rc
                self.process_config()
            return
    ...
    def all_initial_states(self):
        """Iterator over all initial states (available or used)."""
    ...
    def _merge_walkers(self, segments, cumul_weight, bin):
        ...
        glom_weight = cumul_weight[len(segments) - 1]
        iparent = np.digitize((self.rng.uniform(0, glom_weight),), cumul_weight)[0]
        ...
        glom = Segment(
            ...
            weight=glom_weight,
            status=Segment.SEG_STATUS_PREPARED,
            parent_id=gparent_seg.parent_id,
            pcoord=gparent_seg.pcoord.copy(),
            initpoint=gparent_seg.initpoint,
            endpoint=gparent_seg.endpoint,
        )
    ...
    def populate_initial(self, initial_states, weights, system=None):
        ...
        for seg_id, (initial_state, weight) in enumerate(zip(initial_states, weights)):
            dummy_segment = Segment(
                endpoint=initial_state.identifier,
                n_iter=0,
                seg_id=seg_id,
