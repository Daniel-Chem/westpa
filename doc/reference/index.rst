WESTPA reference
================

**Version:** 2026.0a0

**Useful links:** `Home <https://westpa.github.io/westpa>`_
| `Wiki <https://github.com/westpa/westpa/wiki>`_
| `Installation <https://github.com/westpa/westpa/wiki/Installing-WESTPA>`_
| `Source Repository <https://github.com/westpa/westpa>`_
| `Issue Tracker <https://github.com/westpa/westpa/issues>`_
| `Mailing List <https://groups.google.com/g/westpa-users>`_

This reference guide describes WESTPA's command-line interface and Python API.
For tutorials, how-to guides, and other resources for learning how to use WESTPA,
see the `wiki <https://github.com/westpa/westpa/wiki>`_.

.. toctree::
   :maxdepth: 2

   cli/index
   api/index
